...
export default function CreateNewUser() {
  ...
  const {processingUsers} = useSelector(state => state.user);
  ...
  const createUser = async user => {
    ...
    try {
      ...
      dispatch(saveNewUser(editedUser));
    } catch (error) {
        ...
        <SafeAreaView style={{flex: 1}}>
          {modalVisible && (
            <FeedbackModal
              ...
              finalMessage="Usuario criado com sucesso!"
              setModal={() => {
          ...
          <Formik
            ...
            {({
              ...
              <Container>
                ...
                />
                {(JSON.stringify(newUser) !== JSON.stringify(values) ||
                  photo.uri) && (
                  <>
                    <ButtonContainer>
